...
export default function AdminPropertiesClient({ properties: initialProperties = [], globalStats = {}, total = 0, filter = {}, adminId }) {
  ...
  // ========== CORE: BUILD & PUSH URL FROM EXPLICIT VALUES ==========
  // Always pass the new values directly — never read from state,
  // because setState is async and state will be stale at call time.
  ...
  const handleSearchChange = (e) => {
    ...
    debounceTimer.current = setTimeout(() => {
      setPage(1);
      // Pass value directly — state hasn't updated yet
      buildAndPushUrl({ search: value, newPage: 1 });
    }, 400);
  };

  // ========== HANDLE FILTER CHANGE ==========
  // Each setter also immediately fires the URL update with the new value
  ...
  // ========== SYNC PROPERTIES WHEN SERVER RE-RENDERS ==========
  useEffect(() => {
  ...
  const handleApprove = async (propertyId) => {
    ...
    try {
      ...
      setProperties((prev) =>
        prev.map((p) => p._id === propertyId ? { ...p, status: "approved" } : p)
      );
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} size="md" className="max-w-md">
        <Modal.Backdrop>
          <Modal.Container>
            <Modal.Dialog className="bg-white rounded-2xl border-2 border-rose-100/50 shadow-2xl">
              <Modal.Header className="px-6 pt-6 pb-4 border-b border-gray-100">
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Modal.Heading className="text-lg font-bold text-gray-900">Delete Property</Modal.Heading>
                      <p className="text-sm text-gray-500">{selectedProperty?.title}</p>
                    </div>
                  </div>
                  <button onClick={() => setShowDeleteModal(false)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
              ...
              <Modal.Footer className="px-6 pb-6 pt-4 border-t border-gray-100 flex gap-3">
                <Button onPress={() => setShowDeleteModal(false)} className="flex-1 py-3 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition-all duration-300 cursor-pointer">
                  Cancel
                </Button>
                <Button
                ...
                >
                  {deletingId === selectedProperty?._id ? (
                    ...
                    <><Trash2 className="w-4 h-4" strokeWidth={2} /><span>Delete Property</span></>
                  )}
                </Button>
              </Modal.Footer>
            </Modal.Dialog>
        ...
        <Modal.Backdrop>
          <Modal.Container>
            <Modal.Dialog className="bg-white rounded-2xl border-2 border-rose-100/50 shadow-2xl">
              <Modal.Header className="px-6 pt-6 pb-4 border-b border-gray-100">
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Modal.Heading className="text-lg font-bold text-gray-900">Reject Property</Modal.Heading>
                      <p className="text-sm text-gray-500">{selectedProperty?.title}</p>
                    </div>
                  </div>
                  <button onClick={handleCloseRejectModal} className="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
                    <X className="w-5 h-5 text-gray-400" strokeWidth={2} />
                  </button>
                </div>
              </Modal.Header>
              ...
              <Modal.Footer className="px-6 pb-6 pt-4 border-t border-gray-100 flex gap-3">
                <Button onPress={handleCloseRejectModal} className="flex-1 py-3 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition-all duration-300 cursor-pointer">
                  Cancel
                </Button>
                <Button
                ...
                >
                  {isProcessing ? (
                    ...
                    <><XCircle className="w-4 h-4" strokeWidth={2} /><span>Reject Property</span></>
                  )}
                </Button>
              </Modal.Footer>
            </Modal.Dialog>
              ...
              <Modal.Body className="px-6 py-6 overflow-y-auto flex-1">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select className="w-full" value={editFormData.status} onChange={(val) => setEditFormData({ ...editFormData, status: val })} aria-label="Status" classNames={{ trigger: "bg-white border-2 border-blue-200/60 rounded-xl focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all duration-200 shadow-sm hover:border-blue-300 data-[open=true]:border-blue-500", value: "text-gray-800", placeholder: "text-gray-400", indicator: "text-blue-400", popover: "bg-white rounded-xl shadow-lg border border-blue-100/50 mt-1", listBox: "p-1" }}>
                      <Select.Trigger><Select.Value /><Select.Indicator /></Select.Trigger>
                      <Select.Popover>
                        <ListBox>
